/**
 * PNG to SVG Converter — Tool JS (Premium)
 * Features: embed mode (pixel-perfect), trace modes, detail cards, color mode cards, paths slider, preview tabs.
 *
...
(function () {
    ...
    document.querySelectorAll('[data-group="p2svg-detail"] .tc-rsz-mode-card').forEach(function (card) {
        card.addEventListener('click', function () {
            var group = card.closest('[data-group="p2svg-detail"]');
    ...
    document.querySelectorAll('[data-group="p2svg-color"] .tc-rsz-mode-card').forEach(function (card) {
        card.addEventListener('click', function () {
            var group = card.closest('[data-group="p2svg-color"]');
            if (group) group.querySelectorAll('.sel').forEach(function (c) { c.classList.remove('sel'); });
            card.classList.add('sel');
        });
    });
    ...
    if (convertBtn) {
        convertBtn.addEventListener('click', function () {
            ...
            var colorMode = getSelectedVal('p2svg-color') || 'embed';
            if (colorMode === 'embed') {
                doEmbed();
            } else {
                loadLib(doTrace);
    ...
    function doTrace() {
        ...
        var detailKey  = getSelectedVal('p2svg-detail') || 'medium';
        var colorMode  = getSelectedVal('p2svg-color') || 'color';
        var maxPaths   = pathsSlider ? parseInt(pathsSlider.value, 10) : 500;
        var keepTrans  = transChk ? transChk.checked : true;
