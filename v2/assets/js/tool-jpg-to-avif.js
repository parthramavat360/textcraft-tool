...
(function () {
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function () {
            ...
            var base = (nameInput && nameInput.value.trim()) ? nameInput.value.trim().replace(/\.avif$/i, '') : (file ? file.name.replace(/\.jpe?g$/i, '') : 'image');
            TCTP.downloadBlob(convertedBlob, base + '.avif');
        });
    }
