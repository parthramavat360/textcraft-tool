...
(function () {
    ...
    function init() {
        ...
        if (!inp || !out || !genBtn || inp.dataset.tcInit) return;
        inp.dataset.tcInit = '1';

        var resultText = document.getElementById('tc-ps-result-text');

        var currentMode = 'simplified';

        // Mode buttons (PHP renders .tc-modes[data-group="ps-mode"] .tc-btn)
        var group = document.querySelector('.tc-modes[data-group="ps-mode"]');
        if (group && !group.dataset.tcModes) {
            ...
            modeBtns.forEach(function (btn) {
                btn.addEventListener('click', function () {
                    ...
                    currentMode = btn.getAttribute('data-val') || 'simplified';
                });
            });
            var selected = group.querySelector('.tc-btn.sel');
            if (selected) currentMode = selected.getAttribute('data-val') || 'simplified';
        }
        ...
        var copyBtn = document.getElementById('tc-ps-copy');
        if (copyBtn) {
