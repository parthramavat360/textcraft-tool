...
class Widget_Png_To_Jpg extends TextCraft_Base_Widget {
	public function get_name(): string  { return 'textcraft_png_to_jpg'; }
	public function get_title(): string { return esc_html__( 'PNG to JPG Converter', 'textcraft-tools' ); }
	public function get_icon(): string  { return 'eicon-image-rollover'; }

	protected function render_tool_content( array $settings ): void {

		// Drop zone.
		echo '<div id="tc-p2j-drop" role="button" tabindex="0" aria-label="' . esc_attr__( 'Upload PNG images — click to browse or drag and drop to convert to JPG online', 'textcraft-tools' ) . '" '
			. 'class="tc-drop-zone">';
		...
		echo '<p class="tc-drop-desc">' . esc_html__( 'Convert PNG to JPG online for free. Up to 20 PNG images convert with background fill support — all processed in your browser.', 'textcraft-tools' ) . '</p>';
		echo '<p class="tc-drop-desc-sm">' . esc_html__( 'Transparent areas are filled with your chosen background color before JPG export. Your files stay private — no uploads to any server.', 'textcraft-tools' ) . '</p>';
		echo '<input type="file" id="tc-p2j-upload" accept="image/png" multiple class="tc-d-none">';
		echo '</div>';

		// One-line upload progress.
		echo '<div id="tc-p2j-upload-line" class="tc-upload-line">';
		echo '<span id="tc-p2j-upload-text">' . esc_html__( 'Loading images: 0%', 'textcraft-tools' ) . '</span>';
		echo '</div>';

		// Conversion progress.
		echo '<div id="tc-p2j-conv-progress" class="tc-progress-wrap">';
		echo '<div class="tc-d-flex tc-justify-between tc-mb-6">';
		echo '<span id="tc-p2j-conv-label" class="tc-progress-label">' . esc_html__( 'Converting…', 'textcraft-tools' ) . '</span>';
		echo '<span id="tc-p2j-conv-pct" class="tc-progress-pct">0%</span>';
		echo '</div>';
		echo '<div class="tc-progress-bg">';
		echo '<div id="tc-p2j-conv-bar" class="tc-progress-fill tc-progress-fill--orange"></div>';
		echo '</div>';
		echo '</div>';

		// Loaded previews.
		echo '<div id="tc-p2j-previews" class="tc-preview-wrap">';
		echo '<div class="tc-label-row tc-mb-8"><span class="tc-label">' . esc_html__( 'Loaded PNG Images', 'textcraft-tools' ) . '</span></div>';
		echo '<div id="tc-p2j-preview-grid" class="tc-grid-preview-sm"></div>';
		echo '</div>';
		...
		echo '<div>';
		echo '<label class="tc-label tc-d-block tc-mb-8" for="tc-p2j-quality">' . esc_html__( 'Output Image Quality', 'textcraft-tools' ) . '</label>';
		echo '<div class="tc-d-flex tc-items-center tc-gap-10">';
		echo '<input type="range" id="tc-p2j-quality" min="50" max="100" value="92" class="tc-slider">';
		echo '<span id="tc-p2j-quality-val" class="tc-text-14 tc-accent-value tc-min-w-40">92%</span>';
		echo '</div>';
		...
		echo '<div>';
		echo '<label class="tc-label tc-d-block tc-mb-8" for="tc-p2j-bg">' . esc_html__( 'Background Color', 'textcraft-tools' ) . '</label>';
		echo '<div class="tc-d-flex tc-items-center tc-gap-10">';
		echo '<input type="color" id="tc-p2j-bg" value="#ffffff" class="tc-color-picker">';
		echo '<input type="text" id="tc-p2j-bg-text" value="#ffffff" aria-label="' . esc_attr__( 'Background color hex value', 'textcraft-tools' ) . '" class="tc-color-hex">';
		echo '</div>';
		...
		echo esc_html__( 'Your files stay private — all conversions happen locally in your browser with no data uploaded to any server. Previews and converted JPGs are saved in browser cache so they persist after page reload.', 'textcraft-tools' );
		echo '</div>';
		echo '</div>';
		...
		$this->render_button_row(
			[
				[
					'id'       => 'tc-p2j-convert',
					'label'    => '🖼️ ' . esc_html__( 'Convert to JPG', 'textcraft-tools' ),
					'variant'  => 'primary',
					'disabled' => true,
				],
				[
					'id'      => 'tc-p2j-download-all',
					'label'   => '📦 ' . esc_html__( 'Download All (ZIP)', 'textcraft-tools' ),
					'variant' => 'ghost',
				],
				[
					'id'      => 'tc-p2j-clear',
					'label'   => '🗑️ ' . esc_html__( 'Clear All', 'textcraft-tools' ),
					'variant' => 'danger',
				],
			]
		);
		...
		$this->render_stat_bar(
			[
				[ 'id' => 'tc-p2j-stat-loaded',    'label' => esc_html__( 'Files Loaded', 'textcraft-tools' ) ],
				[ 'id' => 'tc-p2j-stat-converted', 'label' => esc_html__( 'Converted', 'textcraft-tools' ) ],
				[ 'id' => 'tc-p2j-stat-size',      'label' => esc_html__( 'Total JPG Size', 'textcraft-tools' ) ],
				[ 'id' => 'tc-p2j-stat-quality',   'label' => esc_html__( 'Quality', 'textcraft-tools' ) ],
			]
		);

		// Results.
		echo '<div id="tc-p2j-cards" class="tc-results-wrap">';
		echo '<div class="tc-label-row"><span class="tc-label">' . esc_html__( 'Converted JPG Files', 'textcraft-tools' ) . '</span></div>';
		echo '<div id="tc-p2j-grid" class="tc-grid-cards-lg"></div>';
		echo '</div>';

		echo '<canvas id="tc-p2j-canvas" class="tc-d-none"></canvas>';
...
(function () {
    ...
    var fileInp     = document.getElementById('tc-p2j-upload');
    var previewWrap = document.getElementById('tc-p2j-previews');
    var previewGrid = document.getElementById('tc-p2j-preview-grid');
    var uploadLine  = document.getElementById('tc-p2j-upload-line');
    var uploadText  = document.getElementById('tc-p2j-upload-text');
    var convWrap    = document.getElementById('tc-p2j-conv-progress');
    var convBar     = document.getElementById('tc-p2j-conv-bar');
    var convPct     = document.getElementById('tc-p2j-conv-pct');
    var convLabel   = document.getElementById('tc-p2j-conv-label');
    var grid        = document.getElementById('tc-p2j-grid');
    var cardsWrap   = document.getElementById('tc-p2j-cards');
    var btnConv     = document.getElementById('tc-p2j-convert');
    var btnDlAll    = document.getElementById('tc-p2j-download-all');
    var btnClear    = document.getElementById('tc-p2j-clear');
    var qualityInp  = document.getElementById('tc-p2j-quality');
    var qualityVal  = document.getElementById('tc-p2j-quality-val');
    var bgInp       = document.getElementById('tc-p2j-bg');
    var bgTextInp   = document.getElementById('tc-p2j-bg-text');
    var canvas      = document.getElementById('tc-p2j-canvas');
    var ctx         = canvas.getContext('2d');

    var files   = [];
    var results = [];

    var DB_NAME  = 'tc_p2j_cache';
    var DB_STORE = 'sessions';
    var DB_KEY   = 'png_to_jpg_session';
    var db       = null;
    ...
    function updateQualityLabel() {
        ...
        document.getElementById('tc-p2j-stat-quality').textContent = qualityInp.value + '%';
    }
    ...
    function resetStats() {
        document.getElementById('tc-p2j-stat-loaded').textContent = '0';
        document.getElementById('tc-p2j-stat-converted').textContent = '0';
        document.getElementById('tc-p2j-stat-size').textContent = '—';
        document.getElementById('tc-p2j-stat-quality').textContent = qualityInp.value + '%';
    }
    ...
    function restoreFromCache() {
        cacheGet(function (cached) {
            ...
            document.getElementById('tc-p2j-stat-loaded').textContent = results.length;
            document.getElementById('tc-p2j-stat-converted').textContent = results.length;
            document.getElementById('tc-p2j-stat-size').textContent = formatSize(results.reduce(function (sum, item) {
    ...
    function addFiles(fileList) {
        ...
        incoming.forEach(function (file) {
            ...
            reader.onload = function (e) {
                ...
                previewWrap.style.display = 'block';
                document.getElementById('tc-p2j-stat-loaded').textContent = files.length;
    ...
    btnConv.addEventListener('click', async function () {
        ...
        var prefix = document.getElementById('tc-p2j-opt-prefix').checked ? 'converted_' : '';
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            document.getElementById('tc-p2j-stat-converted').textContent = String(i + 1);
        }

        document.getElementById('tc-p2j-stat-size').textContent = formatSize(totalSize);
        renderCards(results);
        ...
        btnConv.disabled = false;
        btnConv.textContent = '🖼️ Convert to JPG';
    });
    ...
    function renderCards(items) {
        ...
        items.forEach(function (item) {
            ...
            var savingsText = smaller ? '▼ ' + savings + '% smaller' : '▲ ' + Math.abs(parseFloat(savings)).toFixed(1) + '% larger';
            ...
            card.innerHTML =
                ...
                '<p class="tc-text-10 tc-text-secondary tc-m-0 tc-mb-2">' + item.width + '×' + item.height + '</p>' +
                ...
                '<a href="' + downloadUrl + '" download="' + item.name + '" class="tc-btn tc-btn--primary tc-card-dl-btn">⬇️ Download</a>';
