...
class Widget_Jpg_To_Pdf extends TextCraft_Base_Widget {

    public function get_name(): string {
        return 'textcraft_jpg_to_pdf';
    }

    public function get_title(): string {
        return esc_html__( 'JPG to PDF Converter', 'textcraft-tools' );
    }

    public function get_keywords(): array {
        return [ 'jpg to pdf', 'jpeg to pdf', 'convert image to pdf', 'picture to pdf', 'free online pdf converter' ];
    }
    ...
    protected function render_tool_content( array $settings ): void {
        echo '<div class="tc-jpgtopdf" data-jpg-to-pdf>';

        echo '<div class="tc-jc-drop" role="button" tabindex="0" aria-label="' . esc_attr__( 'Click or drag JPG images to convert them to PDF', 'textcraft-tools' ) . '">';
        echo '<div class="tc-jc-drop__icon">JPG</div>';
        echo '<p class="tc-jc-drop__title">' . esc_html__( 'Click to upload or drag & drop JPG files', 'textcraft-tools' ) . '</p>';
        echo '<p class="tc-jc-drop__hint">' . esc_html__( 'Combine multiple JPG images into a single PDF document — all done securely in your browser', 'textcraft-tools' ) . '</p>';
        echo '<input type="file" class="tc-jc-upload" accept="image/jpeg,image/jpg" multiple>';
        echo '</div>';
        ...
        echo '<label class="tc-label" for="tc-jpgtopdf-quality">' . esc_html__( 'Image Quality', 'textcraft-tools' ) . '</label>';
        echo '<div class="tc-jc-range">';
        echo '<input type="range" id="tc-jpgtopdf-quality" class="tc-jc-quality" min="50" max="100" value="90">';
        echo '<span class="tc-jc-quality-value">90%</span>';
        echo '</div></div>';
        echo '<div class="tc-jc-note">' . esc_html__( 'Higher quality preserves more image detail but creates a larger PDF file. Adjust the slider to find your preferred balance.', 'textcraft-tools' ) . '</div>';
        echo '</div>';
        ...
        echo '<div class="tc-jc-results" data-jpgtopdf-previews hidden>';
        echo '<div class="tc-label-row"><span class="tc-label">' . esc_html__( 'Loaded Images', 'textcraft-tools' ) . '</span></div>';
        echo '<div class="tc-jc-grid tc-jpgtopdf-preview-grid"></div>';
        echo '</div>';

        $this->render_button_row(
            [
                [ 'id' => 'tc-jpgtopdf-convert',      'label' => esc_html__( 'Convert to PDF', 'textcraft-tools' ),      'variant' => 'primary', 'disabled' => true ],
                [ 'id' => 'tc-jpgtopdf-download-pdf', 'label' => esc_html__( 'Download PDF', 'textcraft-tools' ),         'variant' => 'ghost', 'disabled' => true ],
                [ 'id' => 'tc-jpgtopdf-download-all', 'label' => esc_html__( 'Download All Images (ZIP)', 'textcraft-tools' ), 'variant' => 'ghost', 'disabled' => true ],
                [ 'id' => 'tc-jpgtopdf-clear',        'label' => esc_html__( 'Clear All', 'textcraft-tools' ),            'variant' => 'danger' ],
            ]
        );

        $this->render_stat_bar(
            [
                [ 'id' => 'tc-jpgtopdf-stat-loaded', 'label' => esc_html__( 'Images Loaded', 'textcraft-tools' ) ],
                [ 'id' => 'tc-jpgtopdf-stat-pages',  'label' => esc_html__( 'PDF Pages', 'textcraft-tools' ) ],
                [ 'id' => 'tc-jpgtopdf-stat-size',   'label' => esc_html__( 'Output PDF Size', 'textcraft-tools' ) ],
            ]
        );
...
(function () {
    var root = document.querySelector('[data-jpg-to-pdf]');
    if (!root) return;
    ...
    var CACHE_KEY = 'tc_jpgtopdf_session';
    ...
    var previewsWrap  = root.querySelector('[data-jpgtopdf-previews]');
    var previewGrid   = root.querySelector('.tc-jpgtopdf-preview-grid');
    var btnConvert    = document.getElementById('tc-jpgtopdf-convert');
    var btnDownload   = document.getElementById('tc-jpgtopdf-download-pdf');
    var btnDownAll    = document.getElementById('tc-jpgtopdf-download-all');
    var btnClear      = document.getElementById('tc-jpgtopdf-clear');
    var statLoaded    = document.getElementById('tc-jpgtopdf-stat-loaded');
    var statPages     = document.getElementById('tc-jpgtopdf-stat-pages');
    var statSize      = document.getElementById('tc-jpgtopdf-stat-size');
    ...
    function addFiles(fileList) {
        ...
        incoming.forEach(function (file) {
            ...
            reader.onload = function () {
                images.push({ name: file.name, type: file.type || 'image/jpeg', dataUrl: reader.result });
                done += 1;
    ...
    function dataUrlToBlob(dataUrl, fallbackMime) {
        ...
        var mime = mimeMatch ? mimeMatch[1] : (fallbackMime || 'image/jpeg');
    ...
    async function convertToPdf() {
        ...
        for (var i = 0; i < images.length; i++) {
            ...
            var x = (pageW - drawW) / 2;
            var y = (pageH - drawH) / 2;
            pdf.addImage(images[i].dataUrl, 'JPEG', x, y, drawW, drawH, undefined, 'FAST', 0);
        }
    ...
    btnDownAll.addEventListener('click', async function () {
        ...
        images.forEach(function (item, idx) {
            ...
            var ext = /\.jpe?g$/i.test(item.name) ? '' : '.jpg';
            zip.file(item.name || ('image-' + (idx + 1) + ext), blob);
        });
        var content = await zip.generateAsync({ type: 'blob' });
        var link = document.createElement('a');
        link.href = URL.createObjectURL(content);
        link.download = 'jpg-images.zip';
        document.body.appendChild(link);
        link.click();
        link.remove();
        setTimeout(function () { URL.revokeObjectURL(link.href); }, 800);
